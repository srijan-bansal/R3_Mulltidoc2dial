...
def save(model, optimizer, scheduler, step, best_eval_em, best_eval_bleu, opt, dir_path, name):
    ...
    checkpoint = {
        ...
        "opt": opt,
        "best_eval_em": best_eval_em,
        "best_eval_bleu": best_eval_bleu
    }
    torch.save(checkpoint, fp)
    symlink_force(epoch_path, cp)


def load(model_class, dir_path, opt, reset_params=False, load_optim=False, create_optim=True):
    epoch_path = os.path.realpath(dir_path)
    epoch_path = epoch_path.split('/')[-1]
    epoch_path = '/'.join(str(dir_path).split('/')[:-1]) + '/' + epoch_path
    ...
    if load_optim:
        ...
        best_eval_em, best_eval_bleu = 0.0, 0.0
        if "best_eval_em" in checkpoint:
            best_eval_em = checkpoint["best_eval_em"]
        if "best_eval_bleu" in checkpoint:
            best_eval_bleu = checkpoint["best_eval_bleu"]
        if not reset_params:
        ...
        return model, optimizer, scheduler, opt_checkpoint, step, best_eval_em, best_eval_bleu
    if not create_optim:
        return model
    optimizer, scheduler = set_optim(opt, model)
